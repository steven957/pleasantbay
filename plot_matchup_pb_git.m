...
% Set path for input depending on which algorithm is used
switch algthm
    case 'c2rcc'
        %C2RCC
...
for idy=1:mimg
    for ista=1:nsta
        if isempty(pb_match(idy,ista).insitu_sta) || strcmp(pb_match(idy,ista).insitu_sta,'2') % || year(pb_match(idy,ista).insitu_date_time)==2017
            continue
        end
...
%{
hp1=plot(all_chl(:,1),all_chl(:,2),'ko', 'MarkerSize', 8, 'MarkerFaceColor','k');  % Single pixel
xlabel('In situ Chlorophyll','Fontsize',16,'Fontweight','bold','Fontname','Palatino Linotype');
ylabel('Landsat 8 OLI Chlorophyll','Fontsize',16,'Fontweight','bold');
set(gca,'Fontsize',14,'Fontname','Palatino Linotype');
hold on

graph_abs_folder = 'C:\Users\slohrenz\OneDrive - UMASS Dartmouth\Documents\Steve\DOCUMENT\PAPERS\Pleasant Bay\';
print([graph_abs_folder,'graphical_abstract3.tif'],'-dtiff','-r600');
%}
...
str1 = {['N=',num2str(chl_num_obs), '; r^{2}=',num2str(mean(chl_rsquare),3),'; {\itp} = ',num2str(mean(chl_prob),3)]}; %5.85x10^{-10}']};
str2 = {['RMSE = ',num2str(chl_root_mean_sq_error,3)]};

set(gca,'FontSize',14); %,'XTick',[1,2,5,10,20],'YTick',[1,2,5,10,20,50,100]);
...
switch algthm
    ...
    case 'l2gen'        
        % L2gen
        text(1,90,str1,'FontSize',11);
        text(1,70,str2,'FontSize',11);
...
if save_output
    print([InDir,algthm,'_chl_vs_insitu_',matchup_window,'.tif'],'-r600','-dtiff');
end
...
set(gca,'FontSize',14,'YLim',[0 6.5]); %,'XLim',[0 4.5]);
plot([0,5],[0,5],'k:');
...
str1 = {['N=',num2str(secchi_num_obs), '; r^{2}=',num2str(mean(secchi_rsquare),3),'; {\itp} = ',num2str(mean(secchi_prob),3)]}; %2.28x10^{-5}']}; %
str2 = {['RMSE = ',num2str(secchi_root_mean_sq_error,3)]};
text(0.225,6.30,str1,'FontSize',11);
text(0.225,5.891,str2,'FontSize',11);

if save_output
    print([InDir,'secchi_vs_1over',algthm,'kd489_',matchup_window,'.tif'],'-r600','-dtiff');
end
...
switch algthm
    case 'c2rcc'
        ylabel('Landsat 8 C2RCC kd\_z90max (m)', 'Fontsize',14,'Fontname','Arial','Fontweight','Bold');
    case 'c2x'
        ylabel('Landsat 8 C2X kd\_z90max (m)', 'Fontsize',14,'Fontname','Arial','Fontweight','Bold');
    case 'l2gen'
        ylabel('Landsat 8 L2gen 1/Kd_490 (m)', 'Fontsize',14,'Fontname','Arial','Fontweight','Bold');
end
...
if save_output
    print([InDir,'secchi_vs_kd_z90max_',matchup_window,'.tif'],'-r600','-dtiff');
end
