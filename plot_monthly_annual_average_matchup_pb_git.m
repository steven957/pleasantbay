        ...
        for iyr=1:length(year_range)
            sat_indx{imnth} = [sat_indx{imnth};find(isbetween(all_pb_sat.Date_Time,datetime(year_range(iyr),imnth,1),...
                ...
                strcmp(all_pb_sat.Station,num2str(ista2)) & ...
                     all_pb_sat.Date_Time~=datetime('02/21/2022','InputFormat','MM/dd/uuuu') & ...
                     all_pb_sat.Date_Time~=datetime('04/10/2022','InputFormat','MM/dd/uuuu') & ...
                     all_pb_sat.Date_Time~=datetime('07/31/2022','InputFormat','MM/dd/uuuu'))];
...
for ista=[1,3:6,8:17,19:21]
    ...
    if ismember(ista,[1,3,5,6,8,9,10,14,15,19,20])
        text(all_pb_monthly_data.Month(mnth_indx(imnth)),all_pb_monthly_data.conc_chl_mean(mnth_indx(end)),num2str(ista),'FontSize',12,'Color',C(ista,:),...
        'VerticalAlignment','middle','FontWeight','bold');
    else
        text(all_pb_monthly_data.Month(mnth_indx(imnth-1)),all_pb_monthly_data.conc_chl_mean(mnth_indx(end-1)),num2str(ista),'FontSize',12,'Color',C(ista,:),...
        'VerticalAlignment','middle','FontWeight','bold');
    end        
end
...
hleg1 = legend(hp1([1,3:6,8:17,19:21])',sta_char,'NumColumns',5,'Location','Northeast'); %Southoutside');
...
print([InDir1,'c2rcc_chl_insitu_chl_monthly_time_series.tif'],'-r600','-dtiff');
...
set(gca,'Position',axpos,'XLim',[0 6],'YLim',[0 6],'Fontsize',12,'Fontname','Arial');  %,'XScale','log','YScale','log');
plot([0,6],[0,6],'k:');
xlabel('In Situ Chl (mg m^{-3})','Fontsize',14,'Fontname','Arial','Fontweight','Bold')
...
str1 = {['N=',num2str(chl_num_obs), '; r^{2}=',num2str(chl_rsquare,3),'; {\itp} = 9.50x10^{-18}']}; %,num2str(chl_prob,3)]};
str2 = {['RMSE = ',num2str(chl_root_mean_sq_error,3)]};
text(0.225,5.80,str1);
text(0.225,5.520,str2);

print([InDir1,'insitu_chl_vs_sat_conc_chl_monthly.tif'],'-r600','-dtiff');
...
for ista=[1,3:6,8:17,19:21]
    ...
    if ismember(ista,[1,3,4,5,6,8,10,11,12,13,14,20])
        text(all_pb_monthly_data.Month(mnth_indx(5)),1./all_pb_monthly_data.kd489_mean(mnth_indx(5)),num2str(ista),'FontSize',12,'Color',C(ista,:),...
            'VerticalAlignment','middle','FontWeight','bold');
    elseif ismember(ista,[15,16,21])
        text(all_pb_monthly_data.Month(mnth_indx(6)),1./all_pb_monthly_data.kd489_mean(mnth_indx(6)),num2str(ista),'FontSize',12,'Color',C(ista,:),...
            'VerticalAlignment','middle','FontWeight','bold');
    else
        text(all_pb_monthly_data.Month(mnth_indx(4)),1./all_pb_monthly_data.kd489_mean(mnth_indx(4)),num2str(ista),'FontSize',12,'Color',C(ista,:),...
            'VerticalAlignment','middle','FontWeight','bold');
    end
end
...
set(f3,'Position',[293.4444  198.3333  763.1111  618.6667]);

print([InDir1,'1overkd489_insitu_secchi_monthly_time_series.tif'],'-r600','-dtiff');
...
plt_indx = find(all_pb_monthly_data.Station~=2 | all_pb_monthly_data.Station~=7 | all_pb_monthly_data.Station~=18);
hp6=plot(all_pb_monthly_data.insitu_secchi_mean,1./all_pb_monthly_data.kd489_mean,'ko', 'MarkerSize', 5);
hold on 
plt_indx2 = find(all_pb_monthly_data.Station==1);
hp7=plot(all_pb_monthly_data.insitu_secchi_mean(plt_indx2),1./all_pb_monthly_data.kd489_mean(plt_indx2),...
    'bo', 'MarkerSize',10,'LineWidth',1.5);
axpos = get(gca,'Position');
axpos(2)=axpos(2)+0.04;
set(gca,'Position',axpos,'XLim',[1 3.5],'YLim',[1 3.5],'Fontsize',12,'Fontname','Arial');  %,'XScale','log','YScale','log');
plot([1 3.5],[1 3.5],'k:');
xlabel('Secchi Depth (m))','Fontsize',14,'Fontname','Arial','Fontweight','Bold')
...
str3 = {['N=',num2str(secchi_num_obs), '; r^{2}=',num2str(secchi_rsquare,3),'; {\itp} =  2.90x10^{-5}']}; %',num2str(secchi_prob,3)]};
str4 = {['RMSE = ',num2str(secchi_root_mean_sq_error,3)]};
text(1.05,3.425,str3);
text(1.05,3.3,str4);

print([InDir1,'insitu_secchi_vs_sat_kd489_monthly.tif'],'-r600','-dtiff');
